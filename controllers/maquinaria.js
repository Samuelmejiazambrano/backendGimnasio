...
putMaquinariaActivar:async(req,res)=>{
  ...
  const maquinarias = await maquinaria.findByIdAndUpdate(codigo,{estado:1},({new:true}))
  res.json({maquinarias})
...
putMaquinariaDesactivar:async(req,res)=>{    
  const {codigo}=req.params
  const maquinarias = await maquinaria.findByIdAndUpdate(codigo,{estado:0},({new:true}))
  res.json({maquinarias})
